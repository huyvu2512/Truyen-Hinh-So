...
export default function TruyenhinhContent({ initialChannelId } = {}) {
  ...
  useEffect(() => {
    if (!initialChannelId && CHANNELS[0]) {
  ...
  useEffect(() => {
    ...
    fetchEPG();
    // Interval to refresh EPG state every 30 seconds
  ...
  useEffect(() => {
    ...
    // Define handlers at useEffect level so cleanup can reference the same instances
    const handlePlay = () => {
    ...
    if (isDash) {
      ...
      if (Hls.isSupported()) {
        hlsInstance = new Hls({
          // Buffer: load 30s ahead, max 60s
          maxBufferLength: 30,
          maxMaxBufferLength: 60,
          enableWorker: true,
          lowLatencyMode: false,
          // Tell HLS.js to stay 5 segments (~10-15s) behind live edge
          // This reduces how urgently it needs to re-poll the playlist
    ...
    // OPTIMIZATION: Pause HLS loading when tab/page is not visible
    const handleVisibilityChange = () => {
      if (hlsRef.current && document.hidden) {
        hlsRef.current.stopLoad();
      } else if (hlsRef.current && !video.paused) {
    ...
    <div className="flex-1 flex flex-col">
            ...
            {activeChannel ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-[#1b1b22] border border-[#2b2b36] hover:bg-[#252530] text-gray-200 hover:text-white px-4 py-2 rounded-xl transition-all"
                >
                  <svg
        ...
        <section className="epg-sidebar">
          ...
          <div className="epg-list-container relative">
            ...
            {epgList.map((prog) => {
              ...
              const displayTitle = cleanDesc ? `${prog.title}: ${cleanDesc}` : prog.title;
      ...
      <div style={{ width: '100%', padding: '40px 24px 8px 24px' }}>
        <div ref={tabsContainerRef} className="relative flex justify-start gap-8 overflow-x-auto pb-1.5 scrollbar-none">
          {["Tất cả các kênh", "Kênh yêu thích", ...Array.from(new Set(channels.map((ch) => ch.group).filter(Boolean)))].map((cat) => {
